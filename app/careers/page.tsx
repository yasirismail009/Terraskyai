...
function formatSalaryRange(raw: any): string | undefined {
  const currency = (typeof raw?.salary_currency === 'string' && raw.salary_currency) || (typeof raw?.currency === 'string' && raw.currency) || undefined;
...
function normalizeCareerPostSummary(raw: any): CareerPostSummary {
  ...
  return {
    ...
    type: raw?.employment_type_display ?? raw?.employmentTypeDisplay ?? raw?.type ?? raw?.job_type ?? raw?.employment_type ?? raw?.employmentType,
...
function normalizeCareerPostDetail(raw: any): CareerPostDetail {
  ...
  return {
    ...
    requirementsHtml: typeof raw?.requirements === 'string' && looksLikeHtml(raw.requirements) ? raw.requirements : undefined,
    ...
    applicationDeadline: formatDeadline(raw?.applicationDeadline ?? raw?.application_deadline ?? raw?.deadline) ?? (raw?.applicationDeadline ?? raw?.application_deadline ?? raw?.deadline),
...
export default function CareersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.cv) { setSubmitError('Please upload your CV/Resume.'); return; }
    const yesNoToBoolString = (v: '' | 'yes' | 'no') => (v === 'yes' ? 'true' : 'false');
    try {
      setSubmitLoading(true);
      const payload = new FormData();
      if (formData.position && formData.position !== 'general') payload.append('post', formData.position);
      ...
      if (!res.ok) {
        if (maybeJson && typeof maybeJson === 'object') {
          const messages = Object.entries(maybeJson as Record<string, unknown>).map(([k, v]) => {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#FDFCF5" }}>
      ...
      <section style={{
        ...
        <div className="cr-dot-bg" style={{ position:"absolute", inset:0, pointerEvents:"none" }} />
        <div className="cr-glow"  style={{ position:"absolute", top:"-100px", left:"-80px", width:500, height:500, borderRadius:"50%", background:"radial-gradient(circle, rgba(190,169,80,0.1) 0%, transparent 65%)", filter:"blur(50px)", pointerEvents:"none" }} />
        <div className="cr-glow2" style={{ position:"absolute", bottom:"-80px", right:"-60px", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle, rgba(139,94,60,0.09) 0%, transparent 65%)", filter:"blur(50px)", pointerEvents:"none" }} />
        <div className="cr-ring1" style={{ position:"absolute", top:"8%", right:"4%", width:220, height:220, borderRadius:"50%", border:"1px dashed rgba(190,169,80,0.15)", pointerEvents:"none" }} />
        <div className="cr-ring2" style={{ position:"absolute", bottom:"10%", left:"3%", width:160, height:160, borderRadius:"50%", border:"1px dashed rgba(139,94,60,0.13)", pointerEvents:"none" }} />

        <div className="cr-fade" style={{ maxWidth:900, margin:"0 auto", textAlign:"center", position:"relative" }}>
          <span style={{
            display:"inline-block", marginBottom:24,
            background:"rgba(190,169,80,0.12)", border:"1px solid rgba(190,169,80,0.3)",
          ...
          <h1 style={{
            ...
            <em style={{ color:"#BEA950" }}>TerraSkyAI</em>
          </h1>
          <p style={{
            fontFamily:"'DM Sans',sans-serif",
      ...
      {/* ── Open Positions ── */}
      <section className="cr-dot-bg-light" style={{
        background: "#FDFCF5",
        padding: "80px 24px", position:"relative", overflow:"hidden",
      }}>
        <div style={{ position:"absolute", top:-60, right:"8%", width:380, height:380, borderRadius:"50%", background:"radial-gradient(circle, rgba(190,169,80,0.07) 0%, transparent 70%)", filter:"blur(50px)", pointerEvents:"none" }} />
        ...
        <div style={{ maxWidth:900, margin:"0 auto", position:"relative" }}>
          <div className="cr-fade" style={{ marginBottom:48 }}>
            <span style={{
              ...
              letterSpacing:"0.14em", textTransform:"uppercase",
              padding:"5px 16px", borderRadius:999, fontFamily:"'DM Sans',sans-serif",
            }}>Open Positions</span>
            <h2 style={{
              fontFamily:"'Cormorant Garamond',Georgia,serif",
              fontSize:"clamp(2rem,4vw,3rem)", fontWeight:700,
              color:"#2d2e0a", lineHeight:1.15, letterSpacing:"-0.02em", marginBottom:8,
            }}>
          ...
          {positionsLoading && (
            <div style={{ textAlign:"center", padding:"48px 0", fontFamily:"'DM Sans',sans-serif", color:"#9a9878", fontSize:15 }}>
              Loading open positions…
            </div>
          )}

          {!positionsLoading && positionsError && (
            <div style={{ background:"rgba(239,68,68,0.05)", border:"1px solid rgba(239,68,68,0.15)", borderRadius:16, padding:"24px 28px" }}>
              <p style={{ fontFamily:"'DM Sans',sans-serif", color:"#545454", fontSize:14, marginBottom:6 }}>Couldn't load positions right now.</p>
              <p style={{ fontFamily:"'DM Sans',sans-serif", color:"#b91c1c", fontSize:13 }}>{positionsError}</p>
            </div>
          )}

          {!positionsLoading && !positionsError && positionsForRender.length === 0 && (
            <div style={{ background:"#fff", border:"1.5px solid rgba(69,68,17,0.1)", borderRadius:20, padding:"40px 32px", textAlign:"center" }}>
              <p style={{ fontFamily:"'DM Sans',sans-serif", color:"#9a9878", fontSize:15 }}>No open positions at the moment. Please check back soon.</p>
            </div>
          )}
              ...
              return (
                <div key={position.id} className={`cr-job-card${isOpen ? ' is-open' : ''}`}>
                  ...
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:16 }}>
                    <div style={{ flex:1 }}>
                      <h3 style={{
                        ...
                        color:"#2d2e0a", lineHeight:1.2, marginBottom:12,
                      }}>
                    ...
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); handleTogglePosition(position.id); }}
                    >
                  ...
                  {typeof merged.description === 'string' && merged.description.trim().length > 0 && (
                    <div
                      style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#7a7860", marginTop:14, lineHeight:1.7 }}
                  ...
                  {/* Expanded detail */}
                  {isOpen && (
                    <div style={{ marginTop:28, paddingTop:28, borderTop:"1px solid rgba(69,68,17,0.1)" }}>
                      {isDetailLoading && (
                        <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:"#9a9878" }}>Loading details…</p>
                      )}
                      {!isDetailLoading && detailError && (
                        <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:"#b91c1c" }}>{detailError}</p>
                      )}

                      {!isDetailLoading && !detailError && (
                        <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
                          {/* Meta chips */}
                          {(merged.compensation || merged.applicationDeadline || merged.applicationEmail) && (
                            <div style={{ display:"flex", flexWrap:"wrap", gap:12 }}>
                              {merged.compensation && (
                                <div style={{ background:"rgba(69,68,17,0.05)", border:"1px solid rgba(69,68,17,0.12)", borderRadius:12, padding:"10px 16px" }}>
                                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:"#8B5E3C", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:3 }}>Compensation</p>
                                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#2d2e0a", fontWeight:600 }}>{merged.compensation}</p>
                                </div>
                              )}
                              {merged.applicationDeadline && (
                                <div style={{ background:"rgba(69,68,17,0.05)", border:"1px solid rgba(69,68,17,0.12)", borderRadius:12, padding:"10px 16px" }}>
                                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:"#8B5E3C", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:3 }}>Deadline</p>
                                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#2d2e0a", fontWeight:600 }}>{merged.applicationDeadline}</p>
                                </div>
                              )}
                              {merged.applicationEmail && (
                                <div style={{ background:"rgba(69,68,17,0.05)", border:"1px solid rgba(69,68,17,0.12)", borderRadius:12, padding:"10px 16px" }}>
                                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:"#8B5E3C", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:3 }}>Apply Email</p>
                                  <a href={`mailto:${merged.applicationEmail}`} style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#454411", fontWeight:600, textDecoration:"underline" }}>{merged.applicationEmail}</a>
                                </div>
                              )}
                            </div>
                          )}
                          ...
                          {merged.responsibilities && (
                            <div>
                              <h4 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"1.25rem", fontWeight:700, color:"#2d2e0a", marginBottom:12 }}>Key Responsibilities</h4>
                              <ul style={{ display:"flex", flexDirection:"column", gap:8 }}>
                                {merged.responsibilities
                                  .filter(r => !r.includes('Assist with new and existing CV model development through data collection, labeling, ground truthing and validation'))
                                  .map((r, i) => (
                                  <li key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#545454", lineHeight:1.6 }}>
                                    <span style={{ color:"#8B5E3C", marginTop:2, flexShrink:0 }}>▸</span>
                                    <span>{r}</span>
                                  </li>
                                ))}
                              </ul>
                          ...
                          {merged.requirements && (
                            <div>
                              <h4 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"1.25rem", fontWeight:700, color:"#2d2e0a", marginBottom:12 }}>Requirements</h4>
                              <ul style={{ display:"flex", flexDirection:"column", gap:8 }}>
                                {merged.requirements.map((r, i) => (
                                  <li key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#545454", lineHeight:1.6 }}>
                                    <span style={{ color:"#8B5E3C", marginTop:2, flexShrink:0 }}>▸</span>
                                    <span dangerouslySetInnerHTML={{ __html: typeof r === 'string' ? r : String(r) }} />
                                  </li>
                                ))}
                              </ul>
                            </div>
                          ...
                          {!merged.requirements && merged.requirementsHtml && (
                            <div>
                              <h4 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"1.25rem", fontWeight:700, color:"#2d2e0a", marginBottom:12 }}>Requirements</h4>
                              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#545454", lineHeight:1.7 }} dangerouslySetInnerHTML={{ __html: filterDescription(merged.requirementsHtml) }} />
                            </div>
                          )}
                          ...
                          {merged.preferredSkills && (
                            <div>
                              <h4 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:"1.25rem", fontWeight:700, color:"#2d2e0a", marginBottom:12 }}>Preferred Skills</h4>
                              <ul style={{ display:"flex", flexDirection:"column", gap:8 }}>
      ...
      {/* ── Application Form ── */}
      <section id="application-form" className="cr-dot-bg-light" style={{ background:"#FDFCF5", padding:"96px 24px", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:-80, right:"5%", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle, rgba(190,169,80,0.06) 0%, transparent 70%)", filter:"blur(50px)", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:-60, left:"3%", width:320, height:320, borderRadius:"50%", background:"radial-gradient(circle, rgba(139,94,60,0.05) 0%, transparent 70%)", filter:"blur(40px)", pointerEvents:"none" }} />
        ...
        <div style={{ maxWidth:760, margin:"0 auto", position:"relative" }}>
          <div className="cr-fade" style={{ textAlign:"center", marginBottom:56 }}>
            ...
            <h2 style={{
              fontFamily:"'Cormorant Garamond',Georgia,serif",
              fontSize:"clamp(2rem,4vw,3rem)", fontWeight:700,
              color:"#2d2e0a", lineHeight:1.15, letterSpacing:"-0.02em", marginBottom:12,
            }}>
          ...
          <div style={{ background:"#fff", borderRadius:24, padding:"48px 44px", boxShadow:"0 24px 64px rgba(69,68,17,0.1)", border:"1.5px solid rgba(69,68,17,0.08)" }}>
            <form onSubmit={handleSubmit}>
              <div style={{ display:"flex", flexDirection:"column", gap:20 }}>

                {/* Name */}
                <div>
                  <label className="cr-form-label" htmlFor="name">Full Name *</label>
                  <input type="text" id="name" name="name" required value={formData.name} onChange={handleInputChange} placeholder="John Doe" className="cr-form-input" />
                </div>

                {/* Email + Phone */}
                <div className="cr-two-col" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                  <div>
                    <label className="cr-form-label" htmlFor="email">Email Address *</label>
                    <input type="email" id="email" name="email" required value={formData.email} onChange={handleInputChange} placeholder="you@example.com" className="cr-form-input" />
                  </div>
                  <div>
                    <label className="cr-form-label" htmlFor="phone">Phone Number</label>
                    <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+1 (555) 123-4567" className="cr-form-input" />
                  </div>
                </div>

                {/* Position */}
                <div>
                  <label className="cr-form-label" htmlFor="position">Position Applying For *</label>
                  <select id="position" name="position" required value={formData.position} onChange={handleInputChange} className="cr-form-input" style={{ appearance:"none", cursor:"pointer" }}>
                    <option value="">Select a position…</option>
                    {positionsForRender.map((pos) => (
                      <option key={pos.id} value={String(pos.id)}>{pos.title ?? String(pos.id)}</option>
                    ))}
                ...
                {/* Radio questions */}
                <div className="cr-radio-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  {[
                    ...
                    <div key={q.name} className="cr-radio-card">
                      <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:12, fontWeight:700, color:"#454411", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:12 }}>{q.label} *</p>
                      <div style={{ display:"flex", gap:24 }}>
                        {["yes","no"].map((opt) => (
                          <label key={opt} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#545454", fontWeight:500 }}>
                            <input type="radio" name={q.name} value={opt} required checked={q.value === opt} onChange={handleInputChange} style={{ accentColor:"#454411", width:16, height:16 }} />
                ...
                {/* Cover letter */}
                <div>
                  <label className="cr-form-label" htmlFor="coverLetter">Cover Letter</label>
                  <textarea id="coverLetter" name="coverLetter" rows={5} value={formData.coverLetter} onChange={handleInputChange} placeholder="Tell us why you're interested in joining TerraSkyAI…" className="cr-form-input" style={{ resize:"vertical" }} />
                </div>

                {/* CV Upload */}
                <div>
                  <label className="cr-form-label" htmlFor="cv">Upload CV / Resume *</label>
                  <input
                    ref={fileInputRef} type="file" id="cv" name="cv" required
                    accept=".pdf,.doc,.docx" onChange={handleFileChange}
                    className="cr-form-input"
                    style={{ paddingTop:10, paddingBottom:10 }}
                  />
                  <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:"#b0ac90", marginTop:6 }}>Accepted: PDF, DOC, DOCX (Max 5MB)</p>
                </div>

                {/* Submit */}
                <button type="submit" disabled={submitLoading} className="cr-submit-btn" style={{ marginTop:8 }}>
                ...
                {submitError && (
                  <div style={{ background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:12, padding:"14px 18px" }}>
                    <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:"#b91c1c", fontWeight:600 }}>{submitError}</p>
                  </div>
                )}
                {submitSuccess && (
                  <div style={{ background:"rgba(34,197,94,0.06)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:12, padding:"14px 18px" }}>
                    <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:"#15803d", fontWeight:600 }}>{submitSuccess}</p>
                  </div>
                )}
              </div>
            </form>
